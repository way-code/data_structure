...
{
    list->curr = list->point->next;
    printf("[ ");
    while (list->curr != list->point)
    {
...
{
    while (list->point->next != list->point)
    {
        list->curr = list->point->next;
        list->point->next = list->curr->next;
        free(list->curr);
    }
    free(list->point);
    free(list);
...
data_t delete_data(List_t list, int index)
{
    ...
    if (list->point->next == list->point || index >= list->size)
    {
        return (-1);
    }
    move_curr(list, index - 1);
...
List_t new_list(void)
{
    ...
    if (list)
    {
        list->point = (node_t*)malloc(sizeof(node_t));
		if (!list->point)
		{
			return (NULL);
		}
